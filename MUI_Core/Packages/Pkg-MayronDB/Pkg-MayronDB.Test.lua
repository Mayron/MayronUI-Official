...
local type, pairs, string = _G.type, _G.pairs, _G.string;

local function OnStartUp_Test1(self, addOnName) -- luacheck: ignore
  print("OnStartUp_Test1 Started");

  assert(addOnName == "Pkg-MayronDB", "Invalid params!");
...
local function UpdatingToDefaultValueShouldRemoveSavedVariableValue_Test1(self) -- luacheck: ignore
  ...
  assert(obj:IsTable(options), "Should be a table but got "..tostring(options));
...
local function CleaningUpWithNilValue_Test1(self) -- luacheck: ignore
  ...
  self:SetPathValue(self.global, "cleaningTest.subTable1.subTable2[".."hello".."][4]", {
    value = 100,
  });

  assert(self.global.cleaningTest.subTable1.subTable2["hello"][4].value == 100, "does not equal 100");

  self.global.cleaningTest.subTable1.subTable2["hello"][4].value = nil;

  assert(self.global.cleaningTest == nil, "self.global.cleaningTest should be removed!")
