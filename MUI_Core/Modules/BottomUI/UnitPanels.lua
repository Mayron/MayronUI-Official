...
db:AddToDefaults(
  "profile.unitPanels", {
    ...
    sufGradients = { enabled = true; height = 24; targetClassColored = true };
  });
...
function C_UnitPanels:OnInitialize(data, containerModule)
  ...
  db:AppendOnce(
    "profile.unitPanels.sufGradients", nil, {
      ...
      sufGradients = {
        ...
        targetClassColored = function()
          if (data.settings.sufGradients.enabled) then
            local listener = em:GetEventListenerByID(
...
function C_UnitPanels:OnEnable(data)
  ...
  data.right = CreateFrame(
                 "Frame", "MUI_UnitPanelRight", _G.MUI_BottomContainer);
  ...
  data.center.bg = tk:SetBackground(
                     data.center,
                       tk:GetAssetFilePath("Textures\\BottomUI\\Center"));
...
do
  local doubleTextureFilePath = tk:GetAssetFilePath(
                                  "Textures\\BottomUI\\Double");
  local singleTextureFilePath = tk:GetAssetFilePath(
                                  "Textures\\BottomUI\\Single");
