...
local select, math, tinsert, bitband = _G.select, _G.math, _G.table.insert, _G.bit.band;
...
do
  ...
  function Lib:HideLayers(frame, value)
    ...
    if (bitband(value, self.LayerTypes.BACKGROUND) ~= 0) then
      tinsert(layerTypes, "BACKGROUND");
    end

    if (bitband(value, self.LayerTypes.BORDER) ~= 0) then
      tinsert(layerTypes, "BORDER");
    end

    if (bitband(value, self.LayerTypes.ARTWORK) ~= 0) then
      tinsert(layerTypes, "ARTWORK");
    end

    if (bitband(value, self.LayerTypes.OVERLAY) ~= 0) then
      tinsert(layerTypes, "OVERLAY");
    end

    if (bitband(value, self.LayerTypes.HIGHLIGHT) ~= 0) then
